...
option_list <- list(
  optparse::make_option("--fcoding_xls", type = "character", default = NULL,
              help = "Filepath to clinical coding classification systems and maps"),
  optparse::make_option("--f_med_readSR", type = "character", default = "../extdata/dfCodesheetREAD_SR.Coding.RData",
              help = "Filepath to code map beteen READ medication codes and self report medication codes in RData format(included in package)"),optparse::make_option("--fcoding_icd10", type = "character", default = "../extdata/ICD10.coding19.tsv", help = "Filepath to ICD10 coding classification system"),  optparse::make_option("--fcoding_icd9", type = "character", default = "../extdata/ICD9.coding87.tsv",help = "Filepath to ICD9 coding classification system"),optparse::make_option("--fcoding_opcs4", type = "character", default = "../extdata/OPCS4.coding240.tsv",help = "Filepath to OPCS4 coding classification system"),optparse::make_option("--fcoding_20003", type = "character", default = "../extdata/20003.coding4.tsv",help = "Filepath to code map for field 20003"))
...
if (is.null(opt$fcoding_xls)) {
  ...
  stop("Please provide the code maps (.xlsx). Refer resource 592 in Showcase.", call. = FALSE)
}
if (is.null(opt$f_med_readSR)) {
  print_help(opt_parser)
  stop("Please check the path to code map for self-report & READ medications", call. = FALSE)
...
load_data <- function(fcoding.xls,fmed.readSR,ficd10,ficd9,fopcs4,f20003){

  #### READ V2
  dfCodesheet.read_v2_read_ctv3 <- as.data.frame(readxl::read_xlsx(fcoding.xls,sheet="read_v2_read_ctv3"))[,c(2,7)]
  colnames(dfCodesheet.read_v2_read_ctv3)<-c("read_code","CTV3")

  dfCodesheet.read_v2_icd9 <- as.data.frame(readxl::read_xlsx(fcoding.xls,sheet="read_v2_icd9"))[,c(1,2)]
  dfCodesheet.read_v2_icd10 <- as.data.frame(readxl::read_xlsx(fcoding.xls,sheet="read_v2_icd10"))[,c(1,2)]
  dfCodesheet.read_v2_opcs4 <- as.data.frame(readxl::read_xlsx(fcoding.xls,sheet="read_v2_opcs4"))[,c(1,2)]

  dfCodesheet.read_v2_lkp <- as.data.frame(readxl::read_xlsx(fcoding.xls,sheet="read_v2_lkp"))
  ...
  dfCodesheet.read_v2_drugs_lkp<- as.data.frame(readxl::read_xlsx(fcoding.xls,sheet="read_v2_drugs_lkp"))
  dfCodesheet.read_v2_lkp <- rbind(dfCodesheet.read_v2_lkp[,c(1,3)],dfCodesheet.read_v2_drugs_lkp[,1:2])
  dfCodesheet.read_v2_lkp <- as.data.frame(dfCodesheet.read_v2_lkp %>% dplyr::group_by(read_code) %>% dplyr::summarize(text = stringr::str_c(term_description, collapse = "/")))
  #dfCodesheet.read_v2_lkp$read_code <- gsub("\\.","", dfCodesheet.read_v2_lkp$read_code)
  dfCodesheet.read_v2_lkp <- dfCodesheet.read_v2_lkp %>% unique()  %>% dplyr::arrange(read_code)
  dfCodesheet.read_v2_lkp$text <- stringr::str_replace_all(dfCodesheet.read_v2_lkp$text,"[^/[:^punct:]]", "")
  ...
  dfCodesheet.READ$text <- stringr::str_replace_all(dfCodesheet.READ$text,"[^/[:^punct:]]", "")
  ...
  dfCodesheet.read_ctv3_readv2 <- as.data.frame(readxl::read_xlsx(fcoding.xls,sheet="read_ctv3_read_v2"))[,c(1,5)]
  ...
  dfCodesheet.read_ctv3_icd9 <- as.data.frame(readxl::read_xlsx(fcoding.xls,sheet="read_ctv3_icd9"))[,c(1,2)]
  dfCodesheet.read_ctv3_icd10 <- as.data.frame(readxl::read_xlsx(fcoding.xls,sheet="read_ctv3_icd10"))[,c(1,2)]
  dfCodesheet.read_ctv3_opcs4 <- as.data.frame(readxl::read_xlsx(fcoding.xls,sheet="read_ctv3_opcs4"))[,c(1,2)]
  dfCodesheet.read_ctv3_lkp <- as.data.frame(readxl::read_xlsx(fcoding.xls,sheet="read_ctv3_lkp"))[,c(1,2)]
  dfCodesheet.read_ctv3_lkp <- as.data.frame(dfCodesheet.read_ctv3_lkp %>% dplyr::group_by(read_code) %>%  dplyr::summarize(text = stringr::str_c(term_description, collapse = "/")))
  ...
  dfCodesheet.CTV3$text <- stringr::str_replace_all(dfCodesheet.CTV3$text,"[^/[:^punct:]]", "")
  ...
  ####### ICD10
  message("ICD10")
  dfCodesheet.icd10_icd9<- as.data.frame(readxl::read_xlsx(fcoding.xls,sheet="icd9_icd10"))[,c(3,1)]
  colnames(dfCodesheet.icd10_icd9) <- c("ICD10","ICD9")
  dfCodesheet.icd10_lkp <- as.data.frame(readxl::read_xlsx(fcoding.xls,sheet="icd10_lkp"))[,c(2,5)] # not complete (e.g. X*)
  names(dfCodesheet.icd10_lkp) <- c("ICD10","text")
  dfCodesheet.ICD10.coding19 <- data.frame(data.table::fread(ficd10))[,1:2] # <- contains all of the above.
  ...
  dfCodesheet.ICD10$text <- stringr::str_replace_all(dfCodesheet.ICD10$text,"[^/[:^punct:]]", "")
  ...
  message("ICD9")
  dfCodesheet.icd9_lkp <- as.data.frame(readxl::read_xlsx(fcoding.xls,sheet="icd9_lkp")) # certainly not complete!
  colnames(dfCodesheet.icd9_lkp) <- c("ICD9","text")
  dfCodesheet.ICD9.coding87 <- data.frame(data.table::fread(ficd9))[,1:2] # UKB
  ...
  dfCodesheet.ICD9$text <- stringr::str_replace_all(dfCodesheet.ICD9$text,"[^/[:^punct:]]", "")
  dfCodesheet.ICD9 <- data.table::data.table(dfCodesheet.ICD9)
  data.table::setkey(dfCodesheet.ICD9,"ICD9")

  ###### OPCS4
  message("OPSC4")
  dfCodesheet.OPCS4.coding240 <- data.frame(data.table::fread(fopcs4))[,c(1,2)]
  ...
  dfCodesheet.OPCS4$text <- stringr::str_replace_all(dfCodesheet.OPCS4$text,"[^/[:^punct:]]", "")
  dfCodesheet.OPCS4 <- data.table::data.table(dfCodesheet.OPCS4)
  data.table::setkey(dfCodesheet.OPCS4,"OPCS4")

  ### n_20003
  message("f.20003")
  dfCodesheet.n_20003 <- data.frame(data.table::fread(f20003))[,c(1,2)]
  ...
  dfCodesheet.n_20003$text <- stringr::str_replace_all(dfCodesheet.n_20003$text,"[^/[:^punct:]]", "")
...
add.description.to.codes <- function(Str,code.id="UKB.Coding",description.id="Meaning",description.lookuptable=dfCodesheetREAD_SR.Coding,ignore.case=FALSE,firstcodeonly=TRUE) {
  ...
  c<- sapply(VctStr,  function(x){
    ...
    x.d <- stringr::str_replace_all(x.d,  "[^/[:^punct:]]", "") # replace all symbols to not mess up downstream things.
...
add.description.to.vectorofcodes <- function(vctcodes=c("G551.","G55.."),code.id="CTV3",description.id="text",description.lookuptable=LstdfCodesheets$CTV3,ignore.case=FALSE,firstcodeonly=TRUE) {
  ...
  df_c <- df_c %>% dplyr::select(c=eval(code.id),text) %>% unique %>% dplyr::filter(!is.na(c)) %>% dplyr::group_by(c) %>% dplyr::mutate(text = paste0(text, collapse = "/")) %>% unique %>% dplyr::mutate(c_text=paste0(c," (",text,")"))
...
expand_clean_codes <- function(col=df$ICD10,
  ...
  col <- ukbpheno::PreProcessDfDefinitions( df=data.frame(col=col,tmp=rep("NA",length(col))),VctAllColumns = c("col","tmp"),VctColstoupper=NULL)[,1]
...
ui <- shiny::fluidPage(
  ...
  shiny::sidebarLayout(
    ...
    shiny::sidebarPanel(
      ...
      shiny::textAreaInput(inputId="iICD10", label="ICD10", value = "Z951 (cabg)", width = NULL, placeholder = NULL),
      shiny::textAreaInput(inputId="iICD9", label="ICD9", value = "", width = NULL, placeholder = NULL),
      shiny::textAreaInput(inputId="iREAD", label="READ", value = "", width = NULL, placeholder = NULL),
      shiny::textAreaInput(inputId="iCTV3", label="CTV3", value = "", width = NULL, placeholder = NULL),
      shiny::textAreaInput(inputId="iOPCS4", label="OPCS4", value = "K40,K41,K43,K44,K45,K46(cabg),K471(endarterectomy),K49, K50,K75 (pci)", width = NULL, placeholder = NULL),
      shiny::checkboxInput(inputId = "iExpandcodes", "Expand codes, e.g. I50 -> I501,I502, etc.  ", FALSE),
      shiny::actionButton("goButton", "Go!"),
      shiny::HTML("<br><br>Note; this is a tryout version for exploration - translations are not reliable and should be manually verified! (BNF/DMD currently not included.)")
    ),
    ...
    shiny::mainPanel(
      ...
      shiny::tabsetPanel(
        ...
        shiny::tabPanel("output",
                        ...
                        shiny::h4("ICD10"),shiny::textOutput("codes_oICD10"),
                        shiny::h4("ICD9"),shiny::textOutput("codes_oICD9"),
                        shiny::h4("READ"),shiny::textOutput("codes_oREAD"),
                        shiny::h4("CTV3"),shiny::textOutput("codes_oCTV3"),
                        shiny::h4("OPCS4"),shiny::textOutput("codes_oOPCS4"),
                        shiny::h4("n_20003"),shiny::textOutput("codes_on_20003")
                 ),
...
server <- function(input, output) {
    ...
    values_lookup <- shiny::reactiveValues(codes.lookup.shinyready = NULL)

    shiny::observeEvent(input$goButton, {
      shiny::showModal(modalDialog( "please wait" ,easyClose = FALSE,footer=NULL))
